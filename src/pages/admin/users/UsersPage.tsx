...
export default function UsersPage() {
  const { role } = useAuth();
  const { users, isLoading, error, handleEdit, handleRemove } = useUsers(role);
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-background-dark-2 text-gray-500 dark:text-gray-200 text-left text-xs font-semibold uppercase">
            <tr>
              ...
              <th className="px-4 py-3">Reset at</th>
          ...
          <tbody className="bg-white dark:bg-background-dark-2 divide-y divide-gray-100 dark:divide-gray-700">
            {users.map(user => (
              <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-background-dark">
                <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-100">{user.id}</td>
                <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">{user.fullName}</td>
                <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-100">{user.email}</td>
                <td className="px-4 py-3 text-sm text-center text-gray-700 dark:text-gray-100">{user.lives}</td>
                <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-100">{formatDate(user.lives_reset_at)}</td>
                ...
                <td className="px-4 py-3 text-right space-x-2">
                  <button
                    ...
                    className="px-3 py-1 text-sm rounded-md bg-yellow-500 text-white hover:bg-yellow-600 transition"
                  >
                    Edit
                  </button>
                  <button
